...
class MaxHeap:
    ...
    def __getMaxValueChild(self, parentIndex):
        ...
        if not leftChildIndex and not rightChildIndex:
            return None
        elif leftChildIndex and rightChildIndex and self.max_heap[leftChildIndex].value > self.max_heap[rightChildIndex].value:
    ...
    def insert(self, name, value):
        self.max_heap.append(Node(name, value))
        self.maxHeapify()
    ...
    def maxHeapify(self, index=-1):
        ...
        while index != 0:
            ...
            if self.max_heap[parentIndex].value >= self.max_heap[index].value:
                break
    ...
    def bubbleDown(self):
        ...
        while index < len(self.max_heap):
            ...
            if not self.max_heap[index].value < self.max_heap[nextIndex].value:
                break
    ...
    def delete(self, name):
        ...
        for index in range(len(self.max_heap)):
            if self.max_heap[index].name == name:
                node = Node(name, self.max_heap[index].value)
                self.max_heap[index].value = float('+inf')
